...
import { Shield, AlertTriangle, CheckCircle, Info, Globe, Activity, MapPin, HelpCircle } from 'lucide-react';
import Link from 'next/link';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';

const glossaryTips: Record<string, string> = {
  dnssec_enabled: "DNS Security Extensions. Adds a layer of security to DNS by enabling DNS responses to be digitally signed.",
  spf_exists: "Sender Policy Framework. Specifies which mail servers are authorized to send email for your domain.",
  dkim_exists: "DomainKeys Identified Mail. Adds a cryptographic signature to emails to verify authorization.",
  dmarc_policy: "Domain-based Message Authentication, Reporting, and Conformance. Policy for SPF/DKIM failures.",
  mx_resolves: "Mail Exchanger records. Responsible for receiving email.",
  open_resolver: "A DNS server that responds to recursive queries from anyone. A major security risk.",
  zone_transfer_exposed: "AXFR. If exposed, full DNS records can be leaked to anyone.",
  ns_consistency: "Ensures all authoritative nameservers return the same information."
};
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent className="bg-slate-800 border-slate-700 text-slate-200 max-w-xs">
                            <p>{glossaryTips[check.check_name] || "Security check for " + check.check_name}</p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </div>
